 ...
 *
    $dbm = ImbaManagerDatabase::getInstance();
    $dbm->query("SELECT name,value FROM %s WHERE 1;", array($this->$DATABASE_TABLES_SYS_SETTINGS));
    while ($row = $dbm->fetchRow()) {
    public static $SETTINGS[$row['name']] = $row['value'];
    }
 * 
 */
} 
