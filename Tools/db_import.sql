...
CREATE TABLE `oom_openid_multig_category` (`id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,`name` VARCHAR( 100 ) NOT NULL) ENGINE = INNODB;
CREATE TABLE `oom_openid_multig_int_games_cat` (`game_id` int(11) NOT NULL, `cat_id` int(11) NOT NULL, UNIQUE KEY `game_id` (`game_id`,`cat_id`)) ENGINE=INNODB;
...
CREATE TABLE `oom_openid_auth_request` (  `hash` varchar(40) NOT NULL,  `userid` int(10) NOT NULL,  `phpsession` varchar(64) NOT NULL,  `realm` varchar(30) NOT NULL,  `timestamp` int(18) NOT NULL,  `returnto` varchar(255) NOT NULL,  `type` varchar(20) NOT NULL,  `domain` varchar(50) NOT NULL,  `ip` varchar(50) NOT NULL,  UNIQUE KEY `hash` (`hash`)) ENGINE=InnoDB DEFAULT CHARSET=latin1;
